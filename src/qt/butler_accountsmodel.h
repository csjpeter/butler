...
#ifndef BUTLER_ACCOUNTSMODEL_H
#define BUTLER_ACCOUNTSMODEL_H
...
class AccountsModel : public AbstractTableModel
{
private:
	Q_OBJECT
	MY_Q_OBJECT;
...
public:
	...
	const Account& account(int row) NO_FCLOG;
	const AccountSet & accountSet() const { return accounts; };
	void del(int row);
	void addNew(Account & account);
	virtual void update(int row, Account & modified);
	void query();
